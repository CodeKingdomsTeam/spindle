...
( function() {
	...
	var notImplemented = function( node ) {
		CK.Logger.warn( "Not yet implemented for node", node );
		return '';
	};
	...
	CK.threads.translate = function( parseTree, tabOffset ) {
	...
	var translationHandlers = {
		...
		BreakStatement: function( node, tabOffset ) {

			return 'break;';

		},

		ContinueStatement: function( node, tabOffset ) {
		...
		FunctionDeclaration: function( node, tabOffset ) {
			...
			// Get variables
			var variables = CK.threads.variablesForFunctionNode( node );
			...
			text += 'CK.threads.$thread("' + node.id.name + '", ';
			text += 'function*(';
		...
		FunctionExpression: function( node, tabOffset ) {
			...
			var text = '';
			text += 'CK.threads.$thread("function", ';
			text += 'function*( ';
		...
		MemberExpression: function( node, tabOffset ) {
			...
			if ( node.computed ) {
				...
				// TODO: Work out what is global api
		...
		Identifier: function( node, tabOffset ) {
		...
		Literal: function( node, tabOffset ) {
			return node.raw;
		}
